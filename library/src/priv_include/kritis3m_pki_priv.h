...
#define ERROR_OUT(error_code, ...)                                                                 \
        {                                                                                          \
                pki_log(KRITIS3M_PKI_LOG_LEVEL_ERR, __VA_ARGS__);                                  \
                ret = error_code;                                                                  \
                goto cleanup;                                                                      \
        }
...
KRITIS3M_PKI_API int
        importPublicKey(SinglePrivateKey* key, uint8_t const* pubKey, size_t pubKeySize, int type);
KRITIS3M_PKI_API int getSigAlgForKey(SinglePrivateKey* key);
KRITIS3M_PKI_API void freeSinglePrivateKey(SinglePrivateKey* key);

#ifdef HAVE_PKCS11
KRITIS3M_PKI_API int initPkcs11Token(Pkcs11Dev* device,
                                     Pkcs11Token* token,
                                     char const* path,
                                     int slot_id,
                                     uint8_t const* pin,
